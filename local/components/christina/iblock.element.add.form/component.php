...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_par"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST["iblock_submit_par"]))
			{
				...
				else
				{
					...
					{
						$sRedirectUrl = $APPLICATION->GetCurPageParam("", array("edit", "CODE", "strIMessagepar"), $get_index_page=false);
					}
			...
			else
			{
				if (strlen($SEF_URL) > 0)
					...
					$sRedirectUrl = $APPLICATION->GetCurPageParam("edit=Y&CODE=".$arParams["ID"], array("edit", "CODE", "strIMessagepar"), $get_index_page=false);
			}

			$sAction = $sAction == "ADD" ? "ADD" : "EDIT";
			$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."strIMessagepar=";
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = '';
		if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_REQUEST["strIMessagepar"]) && is_string($_REQUEST["strIMessagepar"]))
			$arResult["MESSAGE"] = htmlspecialcharsbx($_REQUEST["strIMessagepar"]);
