			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if(CModule::IncludeModule("imaginweb.sms")) {
						$phone = CIWebSMS::MakePhoneNumber($arUserResult["USER_PHONE"]);

						$msg = 'Вы оформили заказ в магазине THStore.ru. '; //Комплект: '; // $msg .= $basket_string_to_sms;
						$msg .= 'Ваш номер заказа '.$arResult["ORDER_ID"].'. Вы всегда можете изменить метод оплаты и оплатить заказ зайдя в "Личный кабинет" http://www.thstore.ru/personal/';
						CIWebSMS::Send($phone,$msg); 
					}
					...
					foreach ($arResult["BASKET_ITEMS"] as $cItem) {	
						...
						$toEmail["ORDER_LIST"] .= '<tr>
							<td style="border-top: 2px solid #e5e5e5; padding: 13px 0; width: 45px; font-size: 24px; color: #5e5e5e; font-weight: bold; vertical-align: middle;">
								'.intval($cItem["QUANTITY"]).' ×
							</td>
							<td style="border-top: 2px solid #e5e5e5; padding: 13px 0; width: 115px; vertical-align: middle;">
								<img src="http://'.$_SERVER["SERVER_NAME"].CFile::GetPath($cItem["DETAIL_PICTURE"]).'" alt="'.$cItem["NAME"].'" style="display: block;width:92px; height:60px;margin: 0;">
							</td>
							<td style="border-top: 2px solid #e5e5e5; padding: 13px 0; width: 337px; font-size: 14px; font-weight: bold; vertical-align: middle;">
								'.$cItem["NAME"].'
							</td>
							<td style="border-top: 2px solid #e5e5e5; padding: 13px 0; font-size: 14px; font-weight: bold; vertical-align: middle;">
								'.$cItem["PRICE_FORMATED"].'
							</td>
						</tr>';
						
						$obProd = CIBlockElement::GetByID($cItem["PRODUCT_ID"]);
						$arProd = $obProd->GetNext();
						$resArt = CIBlockElement::GetProperty($arProd['IBLOCK_ID'], $arProd['ID'], "sort", "asc", array("CODE" => "pArticle"));
					...
					$toEmail["ORDER_USER_INFO"] .= "ФИО: ".$arUserResult['ORDER_PROP'][1]." ".$arUserResult['ORDER_PROP'][19]." ".$arUserResult['ORDER_PROP'][20]."<br /><br />";
					$toEmail["ORDER_USER_INFO"] .= "Email: ".$arUserResult['ORDER_PROP'][2]."<br /><br />";
					$toEmail["ORDER_USER_INFO"] .= "Телефон: ".$arUserResult['ORDER_PROP'][3]."<br /><br />";
					...
					$eventName = "TH_SALE_ORDER";
					$event = new CEvent;
					$event->Send($eventName, SITE_ID, $toEmail, "N");
					$eventName = "TH_SALE_ORDER_ADMIN";
					$event->Send($eventName, SITE_ID, $toEmail, "N");
