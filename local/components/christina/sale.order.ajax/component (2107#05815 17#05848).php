			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					...
					if(CModule::IncludeModule("imaginweb.sms")) {
						...
						$msg = 'Вы оформили заказ на Th-store. '; //Комплект: '; // $msg .= $basket_string_to_sms;
						$msg .= 'Ваш номер заказа '.$arResult["ORDER_ID"].'. Вы всегда можете изменить метод оплаты и оплатить заказ зайдя в "Личный кабинет" http://www.th-store.ru/personal/';
