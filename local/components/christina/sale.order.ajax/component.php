...
if ($USER->IsAuthorized() || $arParams["ALLOW_AUTO_REGISTER"] == "Y" )
{
	if(strlen($_REQUEST["ORDER_ID"]) <= 0)
	{
		...
		if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["confirmorder"]) && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
		{
					...
					elseif ($arOrderProps["REQUIED"]=="Y")
					{
						if ($arOrderProps["TYPE"]=="TEXT" || $arOrderProps["TYPE"]=="TEXTAREA" || $arOrderProps["TYPE"]=="RADIO" || $arOrderProps["TYPE"]=="SELECT" || $arOrderProps["TYPE"] == "CHECKBOX")
						{
							...
							if($arOrderProps["CODE"] == "PHONE"){
								$arUserResult["USER_PHONE"] = preg_replace("#[^0-9]#", "", $curVal);
							}
						}
			...
			//Ищем пользователей с введёными email и телефоном, если находим то ошибка
			...
			if(!$USER->IsAuthorized()){
				...
				{
					$arResult["ERROR"][] = "Пользователь с такими персональными данными уже существует. Пожалуйста авторизуйтесь.";
					$arResult["POPUP_ERROR"] = "Пользователь с такими персональными данными уже существует. Пожалуйста авторизуйтесь.";
				}else{

					$arFilter = array(
					   'LOGIN' => $arUserResult["USER_PHONE"],
					   'LOGIN_EXACT_MATCH' => 'Y'
					);
					$dbRes = CUser::GetList($by = 'ID', $order = 'ASC', $arFilter);
					if($arUser = $dbRes->Fetch())
					{
						...
						$arFilter = array(
						   'PERSONAL_PHONE' => $arUserResult["USER_PHONE"],
		...
		if($arUserResult["CONFIRM_ORDER"] == "Y" && empty($arResult["ERROR"]))
		{
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					$NEW_LOGIN = $arUserResult["USER_PHONE"];
			...
			if($USER->IsAuthorized() && empty($arResult["ERROR"]))
			{
				...
				if (empty($arResult["ERROR"]))
				{
					...
					$name = strlen($USER->GetFirstName()) > 0 ? ", ".$USER->GetFirstName() : "";
					$emailPhone = preg_replace("#(7)([0-9]{3})([0-9]{3})([0-9]{4})#", "+$1 $2 $3-$4", $arUserResult["USER_PHONE"]);


					$arPaySystem2 = CSalePaySystem::GetByID($arUserResult['PAY_SYSTEM_ID'],$arUserResult['PERSON_TYPE_ID']);

					$toEmail = array(
						"ORDER_ID" => $arResult["ORDER_ID"],
						"ACCOUNT_NUMBER" => $arResult["ORDER_ID"],
						"ORDER_DATE" => Date($DB->DateFormatToPHP(CLang::GetDateFormat("SHORT", SITE_ID))),
						"PRICE" => SaleFormatCurrency($arResult['ORDER_PRICE'], "RUB"),
						"PRICE_ALL" => SaleFormatCurrency($orderTotalSum, "RUB"),
						"SALE_EMAIL" => COption::GetOptionString("main", "order_manager_email"),
						"USER_ID" => $USER->GetID(),
						"NAME" => $name,
						"PHONE" => $emailPhone,
						"EMAIL" => (strlen($arUserResult["USER_EMAIL"])>0 ? $arUserResult["USER_EMAIL"] : $USER->GetEmail()),
						"SOC_INSTA" => COption::GetOptionString("main", "Insta_link"),
						"SOC_VK" => COption::GetOptionString("main", "VK_link"),
						"SOC_FB" => COption::GetOptionString("main", "FB_link"),
						"HEAD_PHONE" => $hPhone,
						"DELIVERY_PRICE" => $arResult["DELIVERY_PRICE"],
						"PAY_SYSTEM" => ""
					...
					$toEmail["ORDER_LIST"] = '';
					$toEmail["ORDER_LIST_UNFORMAT"] = "";

					$arDelivery = CSaleDelivery::GetByID($arUserResult['DELIVERY_ID']);
					$toEmail["DELIVERY"] = $arDelivery['NAME'];

					foreach ($arResult["BASKET_ITEMS"] as $cItem) {
						if(empty($cItem["PREVIEW_PICTURE"])){
							$pict_id=substr($arResult["GRID"]["ROWS"][$cItem["ID"]]['data']["PROPERTY_PMOREPHOTOS_VALUE"],0, strpos($arResult["GRID"]["ROWS"][$cItem["ID"]]["data"]["PROPERTY_PMOREPHOTOS_VALUE"], ','));
							if ($pict_id>0) {
								$arFile = CFile::GetFileArray($pict_id);
								$file = CFile::ResizeImageGet($arFile, array('width'=>92, 'height'=>60), BX_RESIZE_IMAGE_EXACT, true);
								$cItem["DETAIL_PICTURE"] = $pict_id;
							}
						}
						CModule::IncludeModule("iblock");
						$res = CIBlockElement::GetByID($cItem["PRODUCT_ID"]);
						if($ar_res = $res->GetNext()){
							
							$arFile = CFile::GetFileArray($ar_res["DETAIL_PICTURE"]);
							$cItem["DETAIL_PICTURE"] = $arFile["SRC"];

							$cItem["DETAIL_PICTURE"] = str_replace(" ", "%20", $cItem["DETAIL_PICTURE"]);
						}
						$toEmail["ORDER_LIST"] .= '
						<tr style="vertical-align: top;">
							<td style="padding: 19px 0px;border-bottom: 1px solid #ebebeb;"><a href="http://es-auto.ru'.$cItem['DETAIL_PICTURE'].'" style="text-decoration: none;">
							<img src="http://es-auto.ru'.$cItem['DETAIL_PICTURE'].'" width="110" height="70" border="0" style="text-decoration: none; margin: 0px;"></a></td>
							<td colspan="3" style="padding: 17px 0px;border-bottom: 1px solid #ebebeb;">
								<div style="height:45px;overflow:hidden;font-size:12px;line-height:18px;"><a href="'.$cItem["DETAIL_PAGE_URL"].'" style="text-decoration: none;color: #000;">'.$cItem["NAME"].'</a></div>
								<table border="0" cellpadding="0" cellspacing="0" width="100%" style="font-size:13px;line-height:13px;">
									<tr style="vertical-align: top;">
										<td style="padding:0px; width: 170px;">'.$cItem["PRICE_FORMATED"].'</td>
										<td style="padding:0px; width: 70px;">'.intval($cItem["QUANTITY"]).'</td>
										<td style="padding:0px; text-align:right;">'.$cItem["SUM"].'</td>
									</tr>
								</table>
							</td>
						</tr>
						...
						$obProd = CIBlockElement::GetByID($cItem["PRODUCT_ID"]);
						$arProd = $obProd->GetNext();
						$resArt = CIBlockElement::GetProperty($arProd['IBLOCK_ID'], $arProd['ID'], "sort", "asc", array("CODE" => "ARTICLE"));
						if ($obArt = $resArt->GetNext())
						{
							$article = $obArt['VALUE'];
						}

						$toEmail["ORDER_LIST_UNFORMAT"] .= '<p>'.$cItem["NAME"].(strlen($article) > 0 ? ' (артикул '.$article.') ' : "").' - '.intval($cItem["QUANTITY"]).' x '.$cItem["PRICE_FORMATED"].'</p>';

					}

					//Собираем информацию по заказу для менеджеров
					if ($arUserResult['PERSON_TYPE_ID'] == 1) {
						$toEmail["NAME"] = $arUserResult['ORDER_PROP'][2]." ".$arUserResult['ORDER_PROP'][1]." ".$arUserResult['ORDER_PROP'][3];
						$toEmail["ORDER_USER_INFO"] .= "ФИО: ".$arUserResult['ORDER_PROP'][2]." ".$arUserResult['ORDER_PROP'][1]." ".$arUserResult['ORDER_PROP'][3]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Email: ".$arUserResult['ORDER_PROP'][4]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Телефон: ".$arUserResult['ORDER_PROP'][5]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Логин: ".$USER->GetLogin()."<br /><br />";
					}else{
						$toEmail["NAME"] = $arUserResult['ORDER_PROP'][10];
						$toEmail["ORDER_USER_INFO"] .= "Название компании: ".$arUserResult['ORDER_PROP'][10]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Юридический адрес: ".$arUserResult['ORDER_PROP'][11]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "ИНН: ".$arUserResult['ORDER_PROP'][12]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "КПП: ".$arUserResult['ORDER_PROP'][13]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "E-Mail: ".$arUserResult['ORDER_PROP'][14]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Телефон: ".$arUserResult['ORDER_PROP'][15]."<br /><br />";
						$toEmail["ORDER_USER_INFO"] .= "Индекс: ".$arUserResult['ORDER_PROP'][16]."<br /><br />";
					}

					if($arUserResult['ORDER_PROP'][22]=="Y"){
						$toEmail["SERTIFICAT"] = "Пользователь имеет сертификат";
					}else{
						$toEmail["SERTIFICAT"] = "";
					};


					if(strlen($arUserResult['ORDER_PROP'][8]) > 0)
						$toEmail["ORDER_USER_INFO"] .= "Адрес доставки: ".$arUserResult['ORDER_PROP'][8]."<br /><br />";

					$arCountry = CSaleLocation::GetCountryLangByID($arUserResult['DELIVERY_LOCATION']);
					$arCity = CSaleLocation::GetCityLangByID($arUserResult['DELIVERY_LOCATION']);
					$arLocation = CSaleLocation::GetByID($arUserResult['DELIVERY_LOCATION']);
					$toEmail["ORDER_USER_INFO"] .= "Местоположение: ".$arLocation['COUNTRY_NAME_ORIG'].", ".$arLocation['REGION_NAME_ORIG'].", ".$arLocation['CITY_NAME_ORIG']."<br /><br />";

					$toEmail["ORDER_USER_INFO"] .= "Тип плательщика: ".($arUserResult['PERSON_TYPE_ID'] == 1 ? "Физическое лицо" : "Юридическое лицо")."<br /><br />";
					$arPaySystem = CSalePaySystem::GetByID($arUserResult['PAY_SYSTEM_ID'],$arUserResult['PERSON_TYPE_ID']);
					$toEmail["ORDER_USER_INFO"] .= "Способ оплаты: ".$arPaySystem['NAME']."<br /><br />";
					/*вывод платежной системы в новой переменной PAY_SYSTEM*/
					$toEmail["PAY_SYSTEM"] = $arPaySystem['NAME'];
					if ($arPaySystem['NAME'] == "Оплата в банке" || $arPaySystem['NAME'] == "Оплата по карте")
						$toEmail["PAY_LINK"] = '<a href="https://es-auto.ru/personal/order/payment/?ORDER_ID='.$toEmail["ACCOUNT_NUMBER"].'">Ссылка на оплату</a>';


					$arDelivery = CSaleDelivery::GetByID($arUserResult['DELIVERY_ID']);
					$toEmail["ORDER_USER_INFO"] .= "Способ доставки: ".$arDelivery['NAME']."<br /><br />";
					$toEmail["ORDER_USER_INFO"] .= "Комментарий: ".$arUserResult["ORDER_DESCRIPTION"]."<br /><br />";
					$arFilter = Array(
					   //"USER_ID" => $USER->GetID(),
					   "ID" => $arOrder["ACCOUNT_NUMBER"]
					   );

					$db_sales = CSaleOrder::GetList(array("DATE_INSERT" => "ASC"), $arFilter, false, false, array('BASKET_DISCOUNT_COUPON', 'BASKET_DISCOUNT_VALUE', 'ID'));
					while ($ar_sales = $db_sales->Fetch())
					{
						$toEmail["COUPON"] = 'Использован купон: '. $ar_sales['BASKET_DISCOUNT_COUPON'].' - '.$ar_sales['BASKET_DISCOUNT_VALUE'];
					}

					$eventName = "ES_SALE_ORDER";
					$event = new CEvent;
					$event->Send($eventName, SITE_ID, $toEmail, "N");
					$eventName = "ES_SALE_ORDER_ADMIN";
					$event->Send($eventName, SITE_ID, $toEmail, "N");
