...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit_call"]) || !empty($_REQUEST["iblock_apply"])))
	{
	$recaptcha = new \ReCaptcha\ReCaptcha(RE_SEC_KEY);
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST["iblock_submit_call"]))
			{
				...
				else
				{
					...
					{
						$sRedirectUrl = $APPLICATION->GetCurPageParam("", array("edit", "CODE", "strIMessagecall"), $get_index_page=false);
					}
			...
			else
			{
				if (strlen($SEF_URL) > 0)
					...
					$sRedirectUrl = $APPLICATION->GetCurPageParam("edit=Y&CODE=".$arParams["ID"], array("edit", "CODE", "strIMessagecall"), $get_index_page=false);
			}

			$sAction = $sAction == "ADD" ? "ADD" : "EDIT";
			$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&")."strIMessagecall=";
	...
	if ($bShowForm)
	{
		...
		$arResult["MESSAGE"] = '';
		if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_REQUEST["strIMessagecall"]) && is_string($_REQUEST["strIMessagecall"]))
			$arResult["MESSAGE"] = htmlspecialcharsbx($_REQUEST["strIMessagecall"]);
